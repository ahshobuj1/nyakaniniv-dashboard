...
import { useCreateStepMutation, useUpdateStepMutation, useDeleteStepMutation } from '@/features/landing-page/landingPageApi';
import { toast } from 'sonner';
import type { TLandingPageStep } from '@/features/landing-page/types';
...
export function StepsTab({ steps }: StepsTabProps) {
  ...
  const [editingItem, setEditingItem] = useState<TLandingPageStep | null>(null);
  ...
  const handleCreate = async () => {
    try {
      ...
      setIsAddOpen(false);
      setNewItem({ title: '', description: '', order: steps.length + 2 });
      setFile(null);
    } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await updateStep({ id: editingItem.id, data: formData }).unwrap();
  ...
  const performRestoreDefault = async (id: number, order: number) => {
    try {
      const formData = new FormData();
      formData.append('defaultImageUrl', `/home/feature/Feature${order}.png`);
      await updateStep({ id, data: formData }).unwrap();
      toast.success('Default image restored successfully');
    } catch (error) {
  ...
  const executeConfirmAction = async () => {
    ...
    if (confirmDialog.action === 'delete') {
      ...
      await performRestoreDefault(confirmDialog.itemId, confirmDialog.itemOrder || 1);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Manage Steps</h3>
          ...
          <Card key={item.id} className="relative group">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <div className="flex gap-2">
                 <Button variant="ghost" size="sm" onClick={() => setConfirmDialog({ isOpen: true, action: 'restore', itemId: item.id, itemName: item.title, itemOrder: item.order })} className="text-xs mr-2">
                   Restore Default
                 </Button>
      ...
      <ActionConfirmDialog
        ...
        title={confirmDialog.action === 'delete' ? 'Delete Step' : 'Restore Default Image'}
        description={confirmDialog.action === 'delete' ? 'This action cannot be undone.' : 'This will remove the uploaded image and restore the default one.'}
